#ifndef _EROS_SHAPE_H_
#define _EROS_SHAPE_H_
...
class ErosShape : public orsa::TriShape {
 public:
  ErosShape() : TriShape() {
 ...
 public:
  bool read(const std::string & filename) {
    ...
    */
	...
	++count_v;
	// if (count_v <= nv) {
	if (3 == sscanf(line,"%*s %lf %lf %lf",&x,&y,&z)) {
	...
	if (3 == sscanf(line,"%*s %i %i %i",&i,&j,&k)) {
