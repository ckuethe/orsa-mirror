...
public:
    ...
    static void getEllipsoidVV(VertexVector & ellipsoidVertexVector,
        ...
        osg::ref_ptr<orsa::EllipsoidShape> shape =
            new orsa::EllipsoidShape(par->get("a"),
                                     par->get("b"),
                                     par->get("c"));
...
#warning CHOOSE one method here
                ...
                if (!shape->rayIntersection(intersectionPoint,
                                            normal,
                                            orsa::Vector(0,0,0),
                                            (v2e*(vertexVector[k].v-v2e_dv)).normalized(),
                                            false)) {
                    ORSA_DEBUG("problems...");
                    exit(0);
                }
...
int main (int argc, char **argv) {
    ...
    */
    ...
    par->insert("a",orsa::FromUnits(300.0,orsa::Unit::KM),orsa::FromUnits(10.0,orsa::Unit::KM));
    par->insert("b",orsa::FromUnits(300.0,orsa::Unit::KM),orsa::FromUnits(10.0,orsa::Unit::KM));
    par->insert("c",orsa::FromUnits(300.0,orsa::Unit::KM),orsa::FromUnits(10.0,orsa::Unit::KM));
    ...
    if (!converged) {
        ...
        GeoidFitMultimin::getEllipsoidVV(ellipsoidVertexVector,
                                         vertexVector,
                                         par,
                                         true);
