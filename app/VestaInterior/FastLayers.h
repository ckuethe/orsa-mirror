...
// Multifit to make the contribution of the ellipsoid layer match the gravity up to degree 2

class FastLayersMultifit : public orsa::Multifit {
public:
    FastLayersMultifit() :
        orsa::Multifit() { }
public:
    class EllipsoidLayerData {
    public:
        double excessMass;
    };    
public:
    // all input vars
    std::vector<EllipsoidLayerData> ellipsoidLayerData;
    // LayerData::SHLayerVectorType SHLayerVector; // this is never changed
    double totalBodyMass;
    size_t SH_degree;
    size_t SH_size;
    // std::vector< std::vector<mpf_class> > target_norm_C;
    // std::vector< std::vector<mpf_class> > target_norm_S;
    // mpf_class                             target_IzzMR2;
    double R0_plate;
    double R0_gravity;
