...
Paul::t_lmnLMN::~t_lmnLMN() {_instance = 0;}
...
mpz_class Paul::t_lmnLMN::get_mpz(const int l,
    ...
    if ( (l==0) && (m==0) && (n==0) ) {
        if ( (L==0) && (M==0) && (N==0) ) return 1;
        else return 0;
    }

    if ( ((l+L)%2) || ((m+M)%2) || ((n+N)%2) )
        return 0;
...
double Paul::gravitationalPotential(const orsa::PaulMoment * M1,
    ...
    //
    const int M1order = M1->order;
    const int M2order = M2->order;
    //
    unsigned int i5 = 0;
    unsigned int j5 = 0;
    unsigned int k5 = 0;
    ...
    // With optimization
        ...
        int M1M2max = M1order; if (M2order>M1order) M1M2max = M2order;
        long double *expplgamma = new long double[M1M2max+2];
        long double *expmlgamma = new long double[M2order+2];
        for(unsigned int i=0;i<M1M2max+2;i++) { expplgamma[i] = ((long double)(exp( (long double)lgamma((long double)(i))))); }
        for(unsigned int i=0;i<M2order+2;i++) { expmlgamma[i] = ((long double)(exp(-(long double)lgamma((long double)(i))))); }
        double powersign = 0.;

        // Main loop
        for (unsigned int i1=0; i1<=M1order; ++i1) {
            for (unsigned int j1=0; j1<=M1order-i1; ++j1) {
                for (unsigned int k1=0; k1<=M1order-i1-j1; ++k1) {

                    if (M1->M(i1,j1,k1) == 0) continue;

                    for (unsigned int i2=0; i2<=M2order; ++i2) {
                        for (unsigned int j2=0; j2<=M2order-i2; ++j2) {
                            for (unsigned int k2=0; k2<=M2order-i2-j2; ++k2) {
                                ...
                                powersign = 1.; if (((((i1+j1+k1)%2)+2)%2)==1) powersign = -1.;
                                outerSum +=
                                    innerSum *
                                    powersign *
                                    ((double)(M1->M(i1,j1,k1)*M2->M(i2,j2,k2)))*
                                    pow(oneOverR,double(i1+j1+k1+i2+j2+k2+1)) /
...
orsa::Vector Paul::gravitationalForce(const orsa::PaulMoment * M1,
    ...
    FX = l11*Fx + m11*Fy + n11*Fz;
    FY = l21*Fx + m21*Fy + n21*Fz;
    FZ = l31*Fx + m31*Fy + n31*Fz;
    F.set(-FX,-FY,-FZ);
...
orsa::Vector Paul::gravitationalTorque(const orsa::PaulMoment * M1,
    ...
    // rotate back, capital X,Y,Z
    T.set(-(l11*Tx + m11*Ty + n11*Tz),
          -(l21*Tx + m21*Ty + n21*Tz),
          -(l31*Tx + m31*Ty + n31*Tz));
